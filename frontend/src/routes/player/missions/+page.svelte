<script>
  ...
  // Optimization: Pre-compute completed mission IDs in O(M) time to allow O(1) lookups during rendering
  // This avoids O(N*M) performance penalty when evaluating the {#each} loop
