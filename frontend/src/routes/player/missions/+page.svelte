<script>
  ...
  // Optimize O(N^2) rendering bottleneck: pre-compute derived state (O(1) lookups)
  // instead of using array methods like .some() inside the {#each} render block
  $: completedMissionIds = new Set(
    playerMissions.filter(pm => pm.status === 'completed').map(pm => pm.mission_id)
  );
