<script>
  ...
  // Pre-compute a Set of completed mission IDs to achieve O(1) lookup during the {#each} render block
  // instead of calling a helper function with `.some()` which causes an O(N^2) rendering bottleneck.
